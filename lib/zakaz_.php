...
if(empty($err))
{
    ...
    {
        $sql="SELECT * FROM item WHERE CODE2='".$val['id']."'";
        $db->query($sql);
        if($db->getCount()>0)
        {
            $arritem=$db->getArray();
...
//           $sms->sendsms('lyufari','+380954570088',"Заказ с сайта #".$uid." на сумму ".$total);
           $sms->sendsms('lyufari','+380505870147',"Заказ с сайта #".$uid." на сумму ".$total);
           $sms->sendsms('lyufari','+380951727272',"Заказ с сайта #".$uid." на сумму ".$total);
