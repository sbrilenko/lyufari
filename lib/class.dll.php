...
class DLL {
        ...
        if ($this -> _db -> getCount())
        {
            ...
            foreach ($photos as $photo)
            {
                ...
                echo '<div id="block~'.$photo['id'].'" style="width:130px;float:left;margin-bottom:10px">';
                    echo '<img src="/img/products/1000/'.$photo['md5_mictotime']."_".$photo['id'].'.jpg" width="100" style="float:left"/>';
    ...
    /*для массива услуг по номеру (1-баня,2-хамам,3-мини зал)*/
    public function getServicesByType($number)
    {
        $ret=array();
        $sql="SELECT * FROM services WHERE invisible=0 AND type=".$number." ORDER BY priority DESC";
        $this->_db->query($sql);
        if($this->_db->getCount()>0)
        {
