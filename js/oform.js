	...
	$.fn.oform = function(options){
       ...
       else
       {
                ...
                success: function(data) {
                    data=$.parseJSON(data);
                        if(data)
                        {
                            ...
                            $('.price',$this.parent().parent().parent()).text(parseFloat(data['price']*col).toFixed(2))
                            var price=0;
                            $('.price',$this.parent().parent().parent().parent()).each(function()
                            {
                                console.log(parseFloat($(this).text()).toFixed(2))
                                price+=parseFloat($(this).text())
                            })
                            $('.thisusertotal',$this.parent().parent().parent().parent().parent()).text(price.toFixed(2))
                           var allt=0;
                           $('.thisusertotal').each(function()
                           {
                             allt+=parseFloat($(this).text())
                           })
                           $('.alltotal').text(allt.toFixed(2))
                        }
                    }
                };
        ...
        $('.depprouser').on(istouch,function(){
            ...
            var pr=parseFloat($('.price',$(this).parent()).text())
